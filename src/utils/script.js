...
const {fscreen, MyMath} = window;
// import {Stage} from '@/utils/Stage';
import initStage from '@/utils/Stage2';
...
// 8K - can restrict this if needed
	...
	function initWordFire() {
		...
		const pointList = [
			...sayToYou.map((item) => {
				return textToPoints(item, textSizeList[item.length], "Anton");
			}),
		...
		function textToPoints(text, textSize, font) {
			...
			console.log('宽度高度是：', text,textLen, window.screen.height, window.screen.width)
